...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      ball.x += ball.vx || 0;
      ball.y += ball.vy || 0;
      if (ball.x < 20 || ball.x + ball.w > width - 20) ball.vx = -(ball.vx || 0);
      if (ball.y < 20) ball.vy = Math.abs(ball.vy || 0);
      if (hit(ball, player)) ball.vy = -Math.abs(ball.vy || 0);
      s.bricks.forEach((brick) => {
        if (brick.alive && hit(ball, brick)) {
          brick.alive = false;
          ball.vy = -(ball.vy || 0);
          setScoreSafe(score + 15);
        }
      });
      if (ball.y > height) finish(score, "小球掉落，关卡失败。");
      if (s.bricks.every((brick) => !brick.alive)) finish(score + 100, "砖块清空，关卡胜利。", "won");
      drawActor(player, game.accent);
      drawActor(ball, "#fef3c7");
      s.bricks.filter((brick) => brick.alive).forEach((brick) => drawActor(brick, "#38bdf8"));
    };
