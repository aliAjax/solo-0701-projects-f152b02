...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (s.tick % 42 === 0) s.items.push({ x: 30 + Math.random() * (width - 80), y: 0, w: 24, h: 24, vy: 3 + Math.random() * 3 });
      s.items.forEach((item) => item.y += item.vy || 0);
      s.items = s.items.filter((item) => {
        if (hit(item, player)) {
          setScoreSafe(score + 10);
          return false;
        }
        if (item.y > height) {
          loseLife();
      ...
      s.items.forEach((item) => drawActor(item, "#fde68a"));
